...
Заложенный в модуль функционал включает в себя поэтапную автоматическую обработку значений измерений датчика, что повышает надежность и удобство работы. Этот процесс включает в себя [несколько этапов](./README_DATA_REFINE.md#обработка-значений-с-датчика):
...
- Основная, которая состоит из:
    - ветки классов [ClassAncestorSensor](./README_ANCESTOR.md) и [ClassMiddleSensor](./README_MIDDLE.md), хранящих в себе поля и методы, общих для всех датчиков. Реализация класса конкретного датчика выполняется наследованием от **ClassMiddleSensor**;
